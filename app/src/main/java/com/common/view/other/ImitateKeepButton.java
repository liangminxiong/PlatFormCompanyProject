...
public class ImitateKeepButton extends View implements View.OnClickListener {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        //画圆
        ringRadius = mRadius - DensityUtil.dip2px(getContext(), value / 2);
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                angle = 0;
                if (angle == 0) {
                    onViewShortClick.onShortClick(ImitateKeepButton.this);
                    try {
                        Thread.sleep(200);
    ...
    public void onClick(View v) {
        onViewShortClick.onShortClick(v);
    }
    ...
    public void setOnViewShortClick(OnViewShortClick viewClick) {
        this.onViewShortClick = viewClick;
    }
