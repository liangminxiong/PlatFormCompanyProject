...
//                        showLoadingDialog("加载中...");
                    ...
                    protected void onSuccess(GroupDataBean o) {
                        ...
                        if (getView() != null) {
                            if (o.isSuccess()) {
                                if (o.getCode() == 200) {
                                    EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_SUCCESS, o.getData()));
                                } else {
                                    EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_ERROR, o.getMsg()));
                                }
                            } else {
                                EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_ERROR, o.getMsg()));
                            }
                    ...
                    protected void onFail(ApiException e) {
                        dismissLoadingDialog();
                        EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_ERROR, e.getMsg()));
                    }

                    @Override
                    public void onError(Throwable e) {
                        dismissLoadingDialog();
                        EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_ERROR, ""));
                        super.onError(e);
                    }
                    ...
                    protected void _onError(ApiException error) {
                        dismissLoadingDialog();
                        EventBus.getDefault().post(new CommonEvent(Constans.GROUNPINFOS_ERROR, error.getMsg()));
                        super._onError(error);
                    }
                });
    }
